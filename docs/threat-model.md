...
- Do production hosts install a timer/cron job to run `sshfling password prune --all --delete-users` and alert on expired grant metadata?
...
| Expired grant prune | `sshfling password prune --all`, `sshfling password prune --username USER` | Root operator/timer -> local state | Requires exactly one selector, removes expired managed snippets and metadata, and locks or deletes users depending on ownership and flag. | `bin/sshfling::prune_password_grants`, `cmd_password_prune` |
