...
| macOS and Windows package authenticity | IA-5, SC-12, SA-10, SI-7 | CC6.6, CC8.1 | A.8.24, A.8.29 | CIS Controls 2, 4 | Apple Developer ID certificate, notarization output, Authenticode certificate, verification command output | Partial: macOS signing/notarization can be required when external Apple credentials and workflow variables are configured, but release evidence is still required. Windows Authenticode signing is not implemented in source. |
| Secrets and privileged release access | AC-2, AC-3, AC-6, IA-5, PM-12 | CC6.1, CC6.2, CC6.3 | A.5.15, A.5.16, A.5.17, A.5.18, A.8.2 | CIS Controls 5, 6 | Release secret inventory, GitHub environment settings, secret-store access review, rotation records, maintainer offboarding records | Gap: repository docs name required secrets, but live custody and review evidence must come from GitHub or the managed secret store. |
| Security testing and vulnerability management | RA-5, SI-2, SA-11, SA-15, CM-6 | CC7.1, CC7.2, CC8.1 | A.8.8, A.8.25, A.8.28, A.8.29 | CIS Controls 7, 16 | `make release-security-scan`, optional scanner outputs, SBOM, dependency inventory, SAST, shell lint, Dockerfile lint, systemd review | Partial: GHCR publishing now runs baseline source/security/container gates before push, but package release workflows and critical/high vulnerability thresholds still require release-specific evidence, scanner availability, or approved exceptions. |
...
| --- | --- | --- |
| CMP-001 | Production release approval is not proven from repository docs. | Protected tag rule, protected environment approval, release ticket approver, and workflow run IDs. |
...
| CMP-004 | Security scans are not documented as blocking critical/high release risk. | Release policy defining blocking thresholds, scanner outputs, triaged false positives, and exception approvals. |
