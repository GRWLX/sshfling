...
| C91-016-01 | 16 | Swift | SwiftPM | Swift package dependency and executable | library + CLI | sshfling-swift-VERSION.tar.gz | PASS | PASS | PASS | The Ubuntu 24.04 strict catalog records RUNTIME swift PASS with archive-lifecycle mode and the complete SwiftPM library, CLI, removal, and post-removal capability set. |
| C91-016-02 | 16 | Swift | SwiftPM | versioned source-archive publication | source package | sshfling-swift-VERSION.tar.gz | PASS | PASS | PASS | A PASS source-archive row for swift, including inventory digest and repeat-build identity, is recorded in dist/sshfling-systems-languages-VERSION-validation.tsv. |
...
| C91-019-01 | 19 | Dart | pub + npm | compiled server-side adapter | native CLI consumer | sshfling-VERSION.tgz plus sshfling-dart-consumer executable | PASS | PASS | PASS | Dart SDK 3.12.2 completes formatting, analysis, offline resolution, native compilation, and npm run test:dart; the batch reports [PASS] dart. |
...
| C91-085-01 | 85 | Chapel | Mason | Chapel module and executable package | library + CLI | sshfling-chapel-VERSION.tar.gz | PASS | PASS | PASS | The systems-language validator extracts the deterministic archive, runs mason modules, compiles the package and external consumer with chpl, and executes both. |
| C91-085-02 | 85 | Chapel | Mason | versioned source-archive publication | source package | sshfling-chapel-VERSION.tar.gz | PASS | PASS | PASS | A PASS source-archive row for chapel, including inventory digest and repeat-build identity, is recorded in dist/sshfling-systems-languages-VERSION-validation.tsv. |
...
| C91-089-01 | 89 | Ballerina | Ballerina package | Ballerina module dependency | library | sshfling-ballerina-VERSION.tar.gz and grwlx-sshfling-any-VERSION.bala | PASS | PASS | PASS | The functional-language validator runs bal test, bal pack, local repository push, external consumer tests, and removal/import-failure checks. |
...
| cfml-npm-consumer | CFML | npm | server-side CFML adapter project | library consumer | sshfling-VERSION.tgz | `package-web-language-consumers` |
| hack-npm-consumer | Hack | npm | server-side Hack adapter project | library consumer | sshfling-VERSION.tgz | `package-web-language-consumers` |
...
| LD-81-02 | Dart | pub + npm | compiled server-side adapter | native CLI consumer | Package metadata | PASS | pubspec.yaml declares Dart 3 compatibility while package.json pins the packed npm dependency and formatting, analysis, compile, and test commands. |
| LD-81-03 | Dart | pub + npm | compiled server-side adapter | native CLI consumer | Package build | PASS | The web-language batch performs dart format, dart analyze, offline pub resolution, and dart compile exe after installing only the packed SSHFling npm artifact. |
...
| LD-82-01 | Swift | SwiftPM | Swift package dependency and executable | library + CLI | Source surface | PASS | Tracked Swift package sources and its public surface live under packaging/systems-languages/swift. |
| LD-82-02 | Swift | SwiftPM | Swift package dependency and executable | library + CLI | Package metadata | PASS | Package.swift declares the Swift library and executable products while the external consumer uses an explicit local-path package dependency. |
| LD-82-03 | Swift | SwiftPM | Swift package dependency and executable | library + CLI | Package build | PASS | The Ubuntu 24.04 strict systems validator extracts the deterministic archive, builds the package and external consumer with SwiftPM, and executes both. |
| LD-82-04 | Swift | SwiftPM | Swift package dependency and executable | library + CLI | Artifact contents | PASS | The versioned archive contains the SwiftPM manifest, library, executable, consumer project, canonical runtime, templates, and inventory manifest. |
| LD-82-05 | Swift | SwiftPM | Swift package dependency and executable | library + CLI | Isolated consumer | PASS | A separate SwiftPM project imports the extracted SSHFling product without repository-wide package paths. |
...
| LD-84-01 | CFML | npm | server-side CFML adapter project | library consumer | Source surface | PASS | A clean CFML consumer is tracked under packaging/node/consumers/cfml. |
| LD-84-02 | CFML | npm | server-side CFML adapter project | library consumer | Package metadata | PASS | Its package manifest pins the language/framework compiler dependencies and receives the packed SSHFling dependency. |
| LD-84-03 | CFML | npm | server-side CFML adapter project | library consumer | Package build | PASS | packaging/build-web-language-consumers.sh installs the clean consumer and runs its build and test commands. |
| LD-84-04 | CFML | npm | server-side CFML adapter project | library consumer | Artifact contents | PASS | CommandBox executes the CFML template after the Node bridge verifies the packed SSHFling npm API. |
| LD-84-05 | CFML | npm | server-side CFML adapter project | library consumer | Isolated consumer | PASS | The batch validator copies the consumer into a temporary directory and installs only from the packed SSHFling artifact. |
| LD-84-06 | CFML | npm | server-side CFML adapter project | library consumer | Public interface | PASS | The CFML template invokes a template-relative Node bridge from a server-side process boundary. |
| LD-84-07 | CFML | npm | server-side CFML adapter project | library consumer | Version contract | PASS | The consumer invokes the packed library with --version and requires a successful SSHFling status contract. |
| LD-84-08 | CFML | npm | server-side CFML adapter project | library consumer | Runtime assets/workflow | PASS | The consumer checks the bundled template directory while keeping SSH execution in the trusted Node process. |
| LD-85-01 | Hack | npm | server-side Hack adapter project | library consumer | Source surface | PASS | A clean Hack consumer is tracked under packaging/node/consumers/hack. |
| LD-85-02 | Hack | npm | server-side Hack adapter project | library consumer | Package metadata | PASS | Its package manifest pins the language/framework compiler dependencies and receives the packed SSHFling dependency. |
| LD-85-03 | Hack | npm | server-side Hack adapter project | library consumer | Package build | PASS | packaging/build-web-language-consumers.sh installs the clean consumer and runs its build and test commands. |
| LD-85-04 | Hack | npm | server-side Hack adapter project | library consumer | Artifact contents | PASS | HHVM 4.172 executes src/main.hack inside the hhvm/hhvm container with Node v22.23.1 after the Node bridge verifies the packed SSHFling npm API. |
| LD-85-05 | Hack | npm | server-side Hack adapter project | library consumer | Isolated consumer | PASS | The batch validator copies the consumer into a temporary directory and installs only from the packed SSHFling artifact. |
| LD-85-06 | Hack | npm | server-side Hack adapter project | library consumer | Public interface | PASS | The Hack entry point invokes a fixed Node bridge from a server-side HHVM process boundary. |
| LD-85-07 | Hack | npm | server-side Hack adapter project | library consumer | Version contract | PASS | The consumer invokes the packed library with --version and requires a successful SSHFling status contract. |
| LD-85-08 | Hack | npm | server-side Hack adapter project | library consumer | Runtime assets/workflow | PASS | The consumer checks the bundled template directory while keeping SSH execution in the trusted Node process. |
| LD-86-01 | Ballerina | Ballerina package | Ballerina module dependency | library | Source surface | PASS | Tracked Ballerina package sources and its public surface live under packaging/functional-languages/ballerina. |
| LD-86-02 | Ballerina | Ballerina package | Ballerina module dependency | library | Package metadata | PASS | Ballerina.toml declares the grwlx/sshfling package, Ballerina 2201.12.0 distribution, bundled resources, README, and license. |
| LD-86-03 | Ballerina | Ballerina package | Ballerina module dependency | library | Package build | PASS | The functional-language validator runs bal test, bal pack, local repository push, external consumer tests, and removal/import-failure checks. |
| LD-86-04 | Ballerina | Ballerina package | Ballerina module dependency | library | Artifact contents | PASS | The deterministic source archive plus generated BALA contain the public module, tests, canonical runtime, templates, README, and license. |
| LD-86-05 | Ballerina | Ballerina package | Ballerina module dependency | library | Isolated consumer | PASS | A separate Ballerina package resolves grwlx/sshfling from the isolated local repository and imports only the installed package. |
| LD-86-06 | Ballerina | Ballerina package | Ballerina module dependency | library | Public interface | PASS | The module exposes run and runAndCapture APIs for child status and exact stdout validation. |
| LD-86-07 | Ballerina | Ballerina package | Ballerina module dependency | library | Version contract | PASS | The focused Ballerina consumer must print the exact SSHFling release version. |
| LD-86-08 | Ballerina | Ballerina package | Ballerina module dependency | library | Runtime assets/workflow | PASS | The external consumer validates version, init, invalid option, missing runtime, local-repository install, removal, and import absence. |
| LD-87-01 | Chapel | Mason | Chapel module and executable package | library + CLI | Source surface | PASS | Tracked Chapel package sources and its public surface live under packaging/systems-languages/chapel. |
| LD-87-02 | Chapel | Mason | Chapel module and executable package | library + CLI | Package metadata | PASS | Mason.toml declares the Chapel package while the module binds the common C launcher through the verified header. |
| LD-87-03 | Chapel | Mason | Chapel module and executable package | library + CLI | Package build | PASS | The systems-language validator extracts the deterministic archive, runs mason modules, compiles the package and external consumer with chpl, and executes both. |
| LD-87-04 | Chapel | Mason | Chapel module and executable package | library + CLI | Artifact contents | PASS | The source archive contains Chapel sources, Mason metadata, shared launcher sources, runtime, templates, and inventory manifest. |
| LD-87-05 | Chapel | Mason | Chapel module and executable package | library + CLI | Isolated consumer | PASS | A clean consumer imports the extracted SSHFling module outside the repository source tree. |
| LD-87-06 | Chapel | Mason | Chapel module and executable package | library + CLI | Public interface | PASS | The Chapel module exposes launcher version and argument-array execution; the package also builds a CLI. |
| LD-87-07 | Chapel | Mason | Chapel module and executable package | library + CLI | Version contract | PASS | The focused Chapel consumer must print the exact SSHFling release version. |
| LD-87-08 | Chapel | Mason | Chapel module and executable package | library + CLI | Runtime assets/workflow | PASS | The consumer and CLI validate version, init, invalid option, missing runtime, uninstall, and import absence. |
| LD-88-01 | Harbour | hbmk2 | Harbour CLI package | CLI | Source surface | PASS | A Harbour/xBase package source and bridge are tracked under packaging/systems-languages/harbour. |
| LD-88-02 | Harbour | hbmk2 | Harbour CLI package | CLI | Package metadata | PASS | sshfling.hbp declares the terminal target, include path, Harbour source, C bridge, and common launcher source. |
| LD-88-03 | Harbour | hbmk2 | Harbour CLI package | CLI | Package build | PASS | The systems validator extracts the deterministic archive and builds the command with hbmk2 from an isolated source package. |
| LD-88-04 | Harbour | hbmk2 | Harbour CLI package | CLI | Artifact contents | PASS | The source archive contains Harbour source, C bridge, common launcher sources, runtime, templates, and inventory manifest. |
| LD-88-05 | Harbour | hbmk2 | Harbour CLI package | CLI | Isolated consumer | PASS | The validator builds and runs the CLI from a temporary output directory without repository source paths. |
| LD-88-06 | Harbour | hbmk2 | Harbour CLI package | CLI | Public interface | PASS | The Harbour procedure accepts argv with hb_AParams and delegates to the C bridge. |
| LD-88-07 | Harbour | hbmk2 | Harbour CLI package | CLI | Version contract | PASS | The CLI prints the exact SSHFling release version. |
| LD-88-08 | Harbour | hbmk2 | Harbour CLI package | CLI | Runtime assets/workflow | PASS | The CLI validates init assets, invalid option status, missing runtime status, and clean temporary workspace behavior. |
| LD-89-01 | Ring | Ring source package | Ring library and POSIX status-wrapper command | library + CLI | Source surface | PASS | Tracked Ring package sources and its public surface live under packaging/functional-languages/ring. |
| LD-89-02 | Ring | Ring source package | Ring library and POSIX status-wrapper command | library + CLI | Package metadata | PASS | package.ring declares the Ring package metadata while the library, main script, command wrapper, and consumer are tracked separately. |
| LD-89-03 | Ring | Ring source package | Ring library and POSIX status-wrapper command | library + CLI | Package build | PASS | The functional-language validator extracts the deterministic archive, parses package metadata with Ring, and executes a clean external Ring consumer. |
| LD-89-04 | Ring | Ring source package | Ring library and POSIX status-wrapper command | library + CLI | Artifact contents | PASS | The source archive contains Ring source, package metadata, the status-preserving command wrapper, and the canonical runtime bundle. |
| LD-89-05 | Ring | Ring source package | Ring library and POSIX status-wrapper command | library + CLI | Isolated consumer | PASS | A separate Ring consumer loads only the extracted package library outside the source checkout. |
| LD-89-06 | Ring | Ring source package | Ring library and POSIX status-wrapper command | library + CLI | Public interface | PASS | lib.ring exposes argument-list execution; bin/sshfling-ring maps the Ring-reported status to a POSIX command status. |
| LD-89-07 | Ring | Ring source package | Ring library and POSIX status-wrapper command | library + CLI | Version contract | PASS | The focused Ring consumer must print the exact SSHFling release version. |
| LD-89-08 | Ring | Ring source package | Ring library and POSIX status-wrapper command | library + CLI | Runtime assets/workflow | PASS | The consumer and command validate exact version output, init assets, invalid option status, missing runtime status, removal, and import absence. |
| LD-90-01 | Red | Red/System compiler | Red/System source package | CLI | Source surface | PASS | A Red/System package source and CLI entry point are tracked under packaging/systems-languages/red. |
| LD-90-02 | Red | Red/System compiler | Red/System source package | CLI | Package metadata | PASS | package.toml declares the Red/System package name, version, launcher ABI, library source, CLI source, and shared-library dependency. |
| LD-90-03 | Red | Red/System compiler | Red/System source package | CLI | Package build | PASS | The systems validator compiles the CLI with the pinned Red/System toolchain from an isolated source archive. |
| LD-90-04 | Red | Red/System compiler | Red/System source package | CLI | Artifact contents | PASS | The source archive contains Red/System sources, common launcher sources, runtime, templates, and inventory manifest. |
| LD-90-05 | Red | Red/System compiler | Red/System source package | CLI | Isolated consumer | PASS | The validator runs the compiled command from a temporary output directory with a matching 32-bit launcher shared object. |
| LD-90-06 | Red | Red/System compiler | Red/System source package | CLI | Public interface | PASS | The CLI delegates argv handling to the shared launcher ABI through the Red/System import declarations. |
| LD-90-07 | Red | Red/System compiler | Red/System source package | CLI | Version contract | PASS | The CLI prints the exact SSHFling release version. |
| LD-90-08 | Red | Red/System compiler | Red/System source package | CLI | Runtime assets/workflow | PASS | The CLI validates init assets, invalid option status, missing runtime status, and clean temporary workspace behavior. |
| LD-91-01 | Delphi/Object Pascal | Free Pascal units | Object Pascal unit and executable package | library + CLI | Source surface | PASS | A Free Pascal-compatible Object Pascal unit, CLI, and consumer are tracked under packaging/systems-languages/object-pascal. |
| LD-91-02 | Delphi/Object Pascal | Free Pascal units | Object Pascal unit and executable package | library + CLI | Package metadata | PASS | package.toml declares the package identity, Free Pascal compiler boundary, unit source, CLI source, and launcher ABI. |
| LD-91-03 | Delphi/Object Pascal | Free Pascal units | Object Pascal unit and executable package | library + CLI | Package build | PASS | The systems validator compiles the package CLI and isolated consumer with fpc in objfpc mode. |
| LD-91-04 | Delphi/Object Pascal | Free Pascal units | Object Pascal unit and executable package | library + CLI | Artifact contents | PASS | The source archive contains Object Pascal sources, runtime, templates, license, and inventory manifest. |
| LD-91-05 | Delphi/Object Pascal | Free Pascal units | Object Pascal unit and executable package | library + CLI | Isolated consumer | PASS | A clean consumer compiles against the extracted SSHFling unit outside the repository source tree. |
| LD-91-06 | Delphi/Object Pascal | Free Pascal units | Object Pascal unit and executable package | library + CLI | Public interface | PASS | The unit exposes RunSSHFling for argument-array execution and the package includes a CLI. |
| LD-91-07 | Delphi/Object Pascal | Free Pascal units | Object Pascal unit and executable package | library + CLI | Version contract | PASS | The consumer and CLI print the exact SSHFling release version. |
| LD-91-08 | Delphi/Object Pascal | Free Pascal units | Object Pascal unit and executable package | library + CLI | Runtime assets/workflow | PASS | The consumer and CLI validate version, init, invalid option, and missing runtime behavior; Delphi compiler compatibility is not claimed. |
| LD-92-01 | Roc | Roc source package | Roc package and application | library + CLI | Source surface | PASS | A Roc package module, application entry point, and external consumer are tracked under packaging/functional-languages/roc. |
| LD-92-02 | Roc | Roc source package | Roc package and application | library + CLI | Package metadata | PASS | package.roc exposes the SSHFling module while the source package records its versioned module API and basic-cli platform boundary. |
| LD-92-03 | Roc | Roc source package | Roc package and application | library + CLI | Package build | PASS | The functional-language validator runs roc check, roc build, and a clean external Roc consumer against the deterministic source archive. |
| LD-92-04 | Roc | Roc source package | Roc package and application | library + CLI | Artifact contents | PASS | The source archive contains Roc sources, license, README, and the byte-checked canonical runtime bundle. |
| LD-92-05 | Roc | Roc source package | Roc package and application | library + CLI | Isolated consumer | PASS | A separate Roc application imports the extracted package by path and runs outside the repository source tree with explicit runtime resources. |
