...
| LD-85-01 | Ballerina | Ballerina package | Ballerina module dependency | library | Source surface | PASS | Tracked Ballerina package sources and its public surface live under packaging/functional-languages/ballerina. |
| LD-85-02 | Ballerina | Ballerina package | Ballerina module dependency | library | Package metadata | PASS | Ballerina.toml declares the grwlx/sshfling package, Ballerina 2201.12.0 distribution, bundled resources, README, and license. |
| LD-85-03 | Ballerina | Ballerina package | Ballerina module dependency | library | Package build | PASS | The functional-language validator runs bal test, bal pack, local repository push, external consumer tests, and removal/import-failure checks. |
| LD-85-04 | Ballerina | Ballerina package | Ballerina module dependency | library | Artifact contents | PASS | The deterministic source archive plus generated BALA contain the public module, tests, canonical runtime, templates, README, and license. |
| LD-85-05 | Ballerina | Ballerina package | Ballerina module dependency | library | Isolated consumer | PASS | A separate Ballerina package resolves grwlx/sshfling from the isolated local repository and imports only the installed package. |
| LD-85-06 | Ballerina | Ballerina package | Ballerina module dependency | library | Public interface | PASS | The module exposes run and runAndCapture APIs for child status and exact stdout validation. |
| LD-85-07 | Ballerina | Ballerina package | Ballerina module dependency | library | Version contract | PASS | The focused Ballerina consumer must print the exact SSHFling release version. |
| LD-85-08 | Ballerina | Ballerina package | Ballerina module dependency | library | Runtime assets/workflow | PASS | The external consumer validates version, init, invalid option, missing runtime, local-repository install, removal, and import absence. |
| LD-86-01 | Chapel | Mason | Chapel module and executable package | library + CLI | Source surface | PASS | Tracked Chapel package sources and its public surface live under packaging/systems-languages/chapel. |
| LD-86-02 | Chapel | Mason | Chapel module and executable package | library + CLI | Package metadata | PASS | Mason.toml declares the Chapel package while the module binds the common C launcher through the verified header. |
| LD-86-03 | Chapel | Mason | Chapel module and executable package | library + CLI | Package build | PASS | The systems-language validator extracts the deterministic archive, runs mason modules, compiles the package and external consumer with chpl, and executes both. |
| LD-86-04 | Chapel | Mason | Chapel module and executable package | library + CLI | Artifact contents | PASS | The source archive contains Chapel sources, Mason metadata, shared launcher sources, runtime, templates, and inventory manifest. |
| LD-86-05 | Chapel | Mason | Chapel module and executable package | library + CLI | Isolated consumer | PASS | A clean consumer imports the extracted SSHFling module outside the repository source tree. |
| LD-86-06 | Chapel | Mason | Chapel module and executable package | library + CLI | Public interface | PASS | The Chapel module exposes launcher version and argument-array execution; the package also builds a CLI. |
| LD-86-07 | Chapel | Mason | Chapel module and executable package | library + CLI | Version contract | PASS | The focused Chapel consumer must print the exact SSHFling release version. |
| LD-86-08 | Chapel | Mason | Chapel module and executable package | library + CLI | Runtime assets/workflow | PASS | The consumer and CLI validate version, init, invalid option, missing runtime, uninstall, and import absence. |
| LD-87-01 | Harbour | hbmk2 | Harbour CLI package | CLI | Source surface | PASS | A Harbour/xBase package source and bridge are tracked under packaging/systems-languages/harbour. |
| LD-87-02 | Harbour | hbmk2 | Harbour CLI package | CLI | Package metadata | PASS | sshfling.hbp declares the terminal target, include path, Harbour source, C bridge, and common launcher source. |
| LD-87-03 | Harbour | hbmk2 | Harbour CLI package | CLI | Package build | PASS | The systems validator extracts the deterministic archive and builds the command with hbmk2 from an isolated source package. |
| LD-87-04 | Harbour | hbmk2 | Harbour CLI package | CLI | Artifact contents | PASS | The source archive contains Harbour source, C bridge, common launcher sources, runtime, templates, and inventory manifest. |
| LD-87-05 | Harbour | hbmk2 | Harbour CLI package | CLI | Isolated consumer | PASS | The validator builds and runs the CLI from a temporary output directory without repository source paths. |
