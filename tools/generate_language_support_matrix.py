...
PASS_PACKAGE_EVIDENCE = (
    "make test VERSION=0.1.23; make package-deb package-rpm package-dotnet "
    "package-java package-node package-python package-go package-rust package-php "
    "package-ruby package-native-libraries package-perl VERSION=0.1.23; "
    "GitHub release v0.1.23 artifacts"
)
PASS_PYTHON_EVIDENCE = (
    "make package-python VERSION=0.1.23; isolated pip and pipx install/import/CLI/uninstall "
...
PASS_NODE_EVIDENCE = (
    "make test VERSION=0.1.23; make package-node VERSION=0.1.23; npm "
    "CommonJS/ESM import and run, strict TypeScript compile, bin, and uninstall validation"
)
PASS_WEB_CONSUMER_EVIDENCE = (
    "make package-web-language-consumers VERSION=0.1.23; clean npm installation from the "
...
PASS_DART_EVIDENCE = (
    "make package-dart-consumer VERSION=0.1.23; Dart SDK 3.12.2 resolves the packed npm "
...
PASS_CFML_EVIDENCE = (
    "make package-node VERSION=0.1.23; SSHFLING_NPM_PACKAGE=dist/sshfling-0.1.23.tgz "
...
PASS_FUNCTIONAL_LANGUAGES_EVIDENCE = (
    ...
    "dist/sshfling-functional-languages-0.1.23-validation.tsv"
)
PASS_SYSTEM_LANGUAGES_EVIDENCE = (
    "bash packaging/build-systems-languages.sh --allow-blocked; "
    "dist/sshfling-systems-languages-0.1.23-validation.tsv"
)
PASS_SWIFT_EVIDENCE = (
...
PASS_JAVA_EVIDENCE = (
    "make package-java VERSION=0.1.23; Maven clean install, Gradle clean build, "
    "executable/source/Javadocs artifacts, and clean Java, Kotlin, Scala, and Groovy consumers"
)
PASS_JVM_CONSUMER_EVIDENCE = (
    "make package-java VERSION=0.1.23; pinned Maven and Gradle compiler/build metadata, "
...
PASS_CLOJURE_EVIDENCE = (
    "make package-java VERSION=0.1.23; pinned Clojure 1.12 Maven and Gradle projects, "
...
PASS_DOTNET_EVIDENCE = (
    "make package-dotnet VERSION=0.1.23; NuGet global tool plus SSHFling library pack, "
...
PASS_NATIVE_EVIDENCE = (
    "make package-native-libraries VERSION=0.1.23; warning-clean C11/C++17 Ninja/Release "
...
PASS_PERL_EVIDENCE = (
    "make package-perl VERSION=0.1.23; MakeMaker test/dist, archive inspection, isolated "
    "INSTALL_BASE module/CLI execution, init workflow, and removal validation"
)
PASS_GO_EVIDENCE = (
    "make package-go VERSION=0.1.23; go test/vet/install and archive validation in "
    "packaging/build-go.sh"
)
PASS_RUST_EVIDENCE = (
    "make package-rust VERSION=0.1.23; cargo fmt/test/clippy/package/install/uninstall and "
    "publish dry-run evidence"
)
PASS_PHP_EVIDENCE = (
    "make package-php VERSION=0.1.23; Composer validate/archive/install/autoload/CLI/remove "
    "validation in packaging/build-php.sh"
)
PASS_RUBY_EVIDENCE = (
    "make package-ruby VERSION=0.1.23; gem and Bundler install/CLI/uninstall validation in "
    "packaging/build-ruby.sh"
)
PASS_SCRIPT_EVIDENCE = (
    "make test VERSION=0.1.23; shell syntax checks through release validation; "
    "container production test"
)
PASS_SCRIPTING_LANGUAGE_EVIDENCE = (
    "make package-scripting-languages VERSION=0.1.23; versioned archive build, isolated "
...
LANGUAGE_SUPPORT: list[dict[str, str]] = [
    ...
    row(
        "Smalltalk",
        "PASS",
        "GNU Smalltalk package metadata, source, runtime assets, and external consumer are tracked under packaging/functional-languages/smalltalk.",
        PASS_FUNCTIONAL_LANGUAGES_EVIDENCE,
        "GNU Smalltalk validates package distribution, source file-in, external consumer execution, exact version output, init workflow, missing-runtime behavior, removal, and import absence.",
    ),
    ...
    row(
        "APL",
        "PASS",
        "GNU APL package/build metadata, source launcher, runtime assets, and an external consumer are tracked under packaging/scientific-languages/apl.",
        PASS_FUNCTIONAL_LANGUAGES_EVIDENCE,
        "GNU APL executes the package source and external consumer against the canonical runtime with version, init, invalid option, missing runtime, removal, and import-absence checks.",
    ),
    ...
    row(
        "Wolfram Language",
        "PASS",
        "A Mathics3-validated Wolfram Language-compatible package, runner bridge, runtime assets, and external consumer are tracked under packaging/scientific-languages/wolfram-language.",
        PASS_FUNCTIONAL_LANGUAGES_EVIDENCE,
        "Mathics3 executes the package and external consumer against the canonical runtime; Wolfram Engine Paclet conformance remains outside this PASS claim.",
    ),
    ...
    row("Roc", "PASS", "Roc package/build metadata, module source, application entry point, and external consumer are tracked under packaging/functional-languages/roc.", PASS_FUNCTIONAL_LANGUAGES_EVIDENCE, "Roc validates the pinned compiler, source package check/build, external consumer, exact version output, init workflow, exit statuses, and import absence."),
    ...
    row("Raku", "PASS", "Raku META6 package metadata, module source, command wrapper, runtime assets, and external consumer are tracked under packaging/functional-languages/raku.", PASS_FUNCTIONAL_LANGUAGES_EVIDENCE, "Raku validates module import, argv-array process execution through Proc::Async, CLI/runtime behavior, init workflow, invalid option, missing runtime, removal, and import absence."),
    row("Haxe", "PASS", "Haxe haxelib metadata, module source, compile-time package-root macro, Neko command target, runtime assets, and external consumer are tracked under packaging/functional-languages/haxe.", PASS_FUNCTIONAL_LANGUAGES_EVIDENCE, "Haxe validates haxelib metadata, Neko CLI build, external consumer compilation, argv-array process execution, init workflow, invalid option, missing runtime, removal, and import absence."),
    ...
    row("Make",
        ...
        "make test VERSION=0.1.23; package build commands; release readiness commands",
    ...
    row(
        ...
        "docker build -f tests/docker/Dockerfile.production -t sshfling-production-test:0.1.23 .; docker run --rm sshfling-production-test:0.1.23",
